import { Shoe } from './shoe';

export const SHOEDATA: Shoe[] = [{
		"id" : 1,
		"name" : "Nike Roshe Run Premium Holiday ",
		"description" : "Lorem ipsum dolor sit amet, consectetur adipisicing elit. Saepe, vitae sunt sint tempora architecto! Unde repellat rem ipsa, culpa impedit harum fugiat animi, nesciunt corporis cumque magni quo facilis, ipsam.",
		"price" : 150,
		"image" : "timberland.jpg",
		"detail" : "Premium burnished full-grain leather and suede upper Leather is from a tannery rated Silver for its water, energy and waste-management practices"
		
	},
	{
		"id" : 2,
		"name" : "Hookset Handcrafted Chukka",
		"description" : "With its beautiful premium leather, lace-up oxford styling, recycled rubber outsoles and 9-inch height, this Earthkeepers® City Premium style is an undeniably handsome boot. To complement its rustic, commanding outer appearance, we've paid attention to the inside as well - by adding SmartWool® faux shearling to the linings and crafting the footbed using our exclusive anti-fatigue comfort foam technology to absorb shock. Imported.",
		"price" : 109,
		"image" : "nikeshoe.jpg"
		"detail" : "Premium burnished full-grain leather and suede upper Leather is from a tannery rated Silver for its water, energy and waste-management practices"

	},
	{
		"id" : 3,
		"name" : "Nike Roshe Run Premium Holiday ",
		"description" : "Lorem ipsum dolor sit amet, consectetur adipisicing elit. Saepe, vitae sunt sint tempora architecto! Unde repellat rem ipsa, culpa impedit harum fugiat animi, nesciunt corporis cumque magni quo facilis, ipsam.",
		"price" : 150,
		"image" : "nikeshoe.jpg"
		"detail" : "Premium burnished full-grain leather and suede upper Leather is from a tannery rated Silver for its water, energy and waste-management practices"
		
	},
	{
		"id" : 4,
		"name" : "Hookset Handcrafted Chukka",
		"description" : "With its beautiful premium leather, lace-up oxford styling, recycled rubber outsoles and 9-inch height, this Earthkeepers® City Premium style is an undeniably handsome boot. To complement its rustic, commanding outer appearance, we've paid attention to the inside as well - by adding SmartWool® faux shearling to the linings and crafting the footbed using our exclusive anti-fatigue comfort foam technology to absorb shock. Imported.",
		"price" : 109,
		"image" : "nikeshoe.jpg"
		"detail" : "Premium burnished full-grain leather and suede upper Leather is from a tannery rated Silver for its water, energy and waste-management practices"

	}]